...
echo "12. 我的工作区 ▶ "
echo "13. 系统工具 ▶ "
echo -e "\033[33m14. 系统防火墙 ▶ \033[0m"
echo -e "\033[31m15. 定时清理docker内存 ▶ \033[0m"
echo -e "\033[32m16. 安装SSH开启root登录 ▶ \033[0m"
echo -e "\033[34m17. 安装x-ui-reality协议 ▶ \033[0m"
echo -e "\033[32m18. Docker容系统 ▶ \033[0m"
echo "------------------------"
echo "00. 脚本更新日志"
echo "------------------------"
echo "0. 退出脚本"
...
case $choice in
  ...
  while true; do
      echo " ▼ "
      echo "创建Docker容器系统 工具"
      echo "------------------------"
      echo "1. 创建Centos7版本Docker容器"
      echo "2. 创建Debian11版本Docker容器"
      echo "3. 进入Docker容器系统"
      echo "4. 退出Docker容器系统"
      echo "------------------------"
      echo "0. 返回主菜单"
      ...
      case $sub_choice in
          1)
              ...
              if command -v apt &>/dev/null; then
                  docker run -i -t -d --restart=always --privileged=true --name centos7.9 -p 888:888/tcp -p 5432-5452:5432-5452/tcp -p 5432-5452:5432-5452/udp -p 23:22 -p 9443:443 centos:centos7.9.2009 /sbin/init
              ...
              clear
              if command -v apt &>/dev/null; then
                  docker run -i -t -d --restart=always --privileged=true --name debian11 -p 8888:8888/tcp -p 54321-54521:54321-54521/tcp -p 54321-54521:54321-54521/udp -p 21:22 -p 8443:443 debian:latest /sbin/init
              elif command -v yum &>/dev/null; then
                  docker run -i -t -d --restart=always --privileged=true --name debian11 -p 8888:8888/tcp -p 54321-54521:54321-54521/tcp -p 5432-54521:54321-54521/udp -p 21:22 -p 8443:443 debian:latest /sbin/init
              else
                  echo "未知的包管理器!"
              fi
  ...
  while true; do
      echo " ▼ "
      echo "安装ssh开启root登录 工具"
      echo "------------------------"
      echo "1. 开启root登录"
      echo "2. 修改root密码"
      echo "3. 安装SSH"
      echo "4. 编辑sshd_config"
      echo "5. 停止ssh"
      echo "6. 启动ssh+开启启动"
      echo "------------------------"
      echo "0. 返回主菜单"
      ...
      case $sub_choice in
          ...
          2)
              ...
              if command -v apt &>/dev/null; then
                  ...
                  echo '*/1 * * * * /root/ddns.sh' >> docker.cron
                  crontab docker.cron
              elif command -v yum &>/dev/null; then
                  crontab -l > docker.cron
                  echo '*/1 * * * * /root/ddns.sh' >> docker.cron
